...
class User(db.Model):
    ...
    role = db.Column(Enum(UserRole, values_callable=lambda x: x.value), nullable=False, index=True)
    status = db.Column(Enum(UserStatus, values_callable=lambda x: x.value), default=UserStatus.ACTIVE, nullable=False, index=True)
