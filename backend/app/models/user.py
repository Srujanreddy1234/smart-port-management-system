...
class User(db.Model):
    ...
    role = db.Column(Enum(UserRole, values_callable=lambda enum_cls: [e.value for e in enum_cls]), nullable=False, index=True)
    status = db.Column(Enum(UserStatus, values_callable=lambda enum_cls: [e.value for e in enum_cls]), default=UserStatus.ACTIVE, nullable=False, index=True)
