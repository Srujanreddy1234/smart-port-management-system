...
class Report(db.Model):
    ...
    report_type = db.Column(Enum(ReportType, values_callable=lambda enum_cls: [e.value for e in enum_cls]), nullable=False, index=True)
    description = db.Column(db.Text)
    format = db.Column(Enum(ReportFormat, values_callable=lambda enum_cls: [e.value for e in enum_cls]), nullable=False, default=ReportFormat.PDF, index=True)
    status = db.Column(Enum(ReportStatus, values_callable=lambda enum_cls: [e.value for e in enum_cls]), default=ReportStatus.PENDING, nullable=False, index=True)
    schedule = db.Column(Enum(ReportSchedule, values_callable=lambda enum_cls: [e.value for e in enum_cls]), default=ReportSchedule.ON_DEMAND, nullable=False)
...
class ReportTemplate(db.Model):
    ...
    default_format = db.Column(Enum(ReportFormat, values_callable=lambda enum_cls: [e.value for e in enum_cls]), default=ReportFormat.PDF)
