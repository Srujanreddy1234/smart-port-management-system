...
class Invoice(db.Model):
    ...
    status = db.Column(Enum(InvoiceStatus, values_callable=lambda x: x.value), default=InvoiceStatus.DRAFT, nullable=False, index=True)
    payment_status = db.Column(db.String(50), default='Unpaid')
    payment_method = db.Column(Enum(PaymentMethod, values_callable=lambda x: x.value))
...
class BillingLine(db.Model):
    ...
    category = db.Column(Enum(BillingCategory, values_callable=lambda x: x.value), nullable=False, index=True)
