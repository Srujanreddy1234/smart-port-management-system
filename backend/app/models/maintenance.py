...
class Equipment(db.Model):
    ...
    equipment_type = db.Column(Enum(EquipmentType, values_callable=lambda x: x.value), nullable=False, index=True)
    ...
    status = db.Column(Enum(EquipmentStatus, values_callable=lambda x: x.value), default=EquipmentStatus.OPERATIONAL, nullable=False, index=True)
...
class MaintenanceSchedule(db.Model):
    ...
    maintenance_type = db.Column(Enum(MaintenanceType, values_callable=lambda x: x.value), nullable=False, index=True)
    priority = db.Column(Enum(MaintenancePriority, values_callable=lambda x: x.value), default=MaintenancePriority.MEDIUM, nullable=False, index=True)
    status = db.Column(Enum(MaintenanceStatus, values_callable=lambda x: x.value), default=MaintenanceStatus.SCHEDULED, nullable=False, index=True)
    ...
    next_maintenance_type = db.Column(Enum(MaintenanceType, values_callable=lambda x: x.value))
...
class ServiceLog(db.Model):
    ...
    service_type = db.Column(Enum(MaintenanceType, values_callable=lambda x: x.value), nullable=False, index=True)
    title = db.Column(db.String(255), nullable=False)
    description = db.Column(db.Text)
    ...
    next_service_type = db.Column(Enum(MaintenanceType, values_callable=lambda x: x.value))
