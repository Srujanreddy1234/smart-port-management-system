...
class Ship(db.Model):
    ...
    vessel_type = db.Column(Enum(VesselType, values_callable=lambda enum_cls: [e.value for e in enum_cls]), nullable=False, index=True)
    ...
    status = db.Column(Enum(ShipStatus, values_callable=lambda enum_cls: [e.value for e in enum_cls]), default=ShipStatus.SCHEDULED, nullable=False, index=True)
...
class Berth(db.Model):
    ...
    status = db.Column(Enum(BerthStatus, values_callable=lambda enum_cls: [e.value for e in enum_cls]), default=BerthStatus.AVAILABLE, nullable=False, index=True)
