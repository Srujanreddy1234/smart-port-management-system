...
class SecurityIncident(db.Model):
    ...
    incident_type = db.Column(Enum(IncidentType, values_callable=lambda enum_cls: [e.value for e in enum_cls]), nullable=False, index=True)
    zone = db.Column(Enum(SecurityZone, values_callable=lambda enum_cls: [e.value for e in enum_cls]), nullable=False, index=True)
    severity = db.Column(Enum(IncidentSeverity, values_callable=lambda enum_cls: [e.value for e in enum_cls]), nullable=False, index=True)
    status = db.Column(Enum(IncidentStatus, values_callable=lambda enum_cls: [e.value for e in enum_cls]), default=IncidentStatus.ACTIVE, nullable=False, index=True)
...
class Alert(db.Model):
    ...
    alert_type = db.Column(Enum(AlertType, values_callable=lambda enum_cls: [e.value for e in enum_cls]), nullable=False, index=True)
    title = db.Column(db.String(255), nullable=False)
    message = db.Column(db.Text)
    zone = db.Column(Enum(SecurityZone, values_callable=lambda enum_cls: [e.value for e in enum_cls]), index=True)
    camera_id = db.Column(db.String(50))
    severity = db.Column(Enum(IncidentSeverity, values_callable=lambda enum_cls: [e.value for e in enum_cls]), default=IncidentSeverity.MEDIUM, nullable=False, index=True)
