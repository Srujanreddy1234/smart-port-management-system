...
class SecurityIncident(db.Model):
    ...
    incident_type = db.Column(Enum(IncidentType), nullable=False, index=True)
    zone = db.Column(Enum(SecurityZone), nullable=False, index=True)
    severity = db.Column(Enum(IncidentSeverity), nullable=False, index=True)
    status = db.Column(Enum(IncidentStatus), default=IncidentStatus.ACTIVE, nullable=False, index=True)
...
class Alert(db.Model):
    ...
    alert_type = db.Column(Enum(AlertType), nullable=False, index=True)
    title = db.Column(db.String(255), nullable=False)
    message = db.Column(db.Text)
    zone = db.Column(Enum(SecurityZone), index=True)
    camera_id = db.Column(db.String(50))
    severity = db.Column(Enum(IncidentSeverity), default=IncidentSeverity.MEDIUM, nullable=False, index=True)
